...
class PromptCompressor:
    ...
    def preserves_critical_content(cls,original,compressed):
        compressed=str(compressed or ""); return all(fragment in compressed for fragment in cls.critical_fragments(original))

    @staticmethod
    def _generic_compression_prompt(prompt):
        return (
            "Compress the following common task specification to reduce repeated context cost. "
            "Do not add new requirements. Preserve every code block, number, filename/path, error/exception, "
    ...
    def _invoke(cls,prompt,utility_agent):
        ...
        if hasattr(utility_agent,"compress_prompt"):
            ...
            if not isinstance(result,dict): return None,"utility_invalid_response"
    ...
    def compress(cls,prompt,utility_agent):
        ...
        if result is None:
            return {"original":prompt,"compressed":prompt,"applied":False,"fallback_reason":reason,"original_tokens":0,"compressed_tokens":0,"saved_tokens":0,"saved_percent":0}
        candidate=result.get("text",prompt); original_tokens=result.get("original_tokens",0); compressed_tokens=result.get("compressed_tokens",0)
