...
def select_work_mode(prompt,requested_mode="thinking",policy="manual"):
    ...
    if policy not in AUTO_VALUES: return {"policy":"manual","requested":requested,"selected":requested if requested in WORK_MODES else "thinking","reason":"manual_override"}
    terms=_terms(prompt); scores={"coding":len(terms&_CODING),"research":len(terms&_RESEARCH),"thinking":len(terms&_THINKING)}; best=max(scores,key=lambda n:(scores[n],-WORK_MODES.index(n)))
...
def retrieve_relevant_chats(rows,query,limit=6):
    ...
    ranked.sort(key=lambda item:(item[0],item[1]),reverse=True); return [row for _,_,row in ranked[:max(0,int(limit))]]
def format_retrieved_context(rows,max_chars=5000):
    return "\n\n".join(f"[session:{row.get('session_id','')}] Q: {str(row.get('prompt') or '')[:500]}\nA: {str(row.get('final_answer') or '')[:900]}" for row in rows or [])[:max_chars]
