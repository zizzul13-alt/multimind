...
class IdentityFirstApplication(MultiMindApplication):
    ...
    def _identity_capability_registry(self):
        ...
        return CapabilityRegistry(capabilities={identity_id: {"coding", "research", "thinking"} for identity_id in specs})
    ...
    def capability_state(self, mode):
        ...
        return {
            "mode": registry.normalize_mode(mode),
            "participants": [state.as_dict() for state in states],
            "recommended": registry.recommended(mode, identities),
        }
    ...
    def _route(self, request, final_prompt, context):
        ...
        for identity_id in request.active_agents:
            ...
            if not slot:
                # Existing DebateOrchestrator has six participant slots. Fail
                # closed rather than overwrite another selected identity.
        ...
        orchestrator = self.debate_factory(
            gemini_agent=execution_agents.get("gemini"), deepseek_agent=execution_agents.get("deepseek"),
            groq_agent=execution_agents.get("groq"), cloudflare_agent=execution_agents.get("cloudflare"),
            openrouter_agent=execution_agents.get("openrouter"), huggingface_agent=execution_agents.get("huggingface"),
        )
        try:
        ...
        result["identity_routing"] = {
            "mode": "identity_first", "cross_identity_fallback": "disabled",
            "routes": {identity_id: list(specs[identity_id].route_order) for identity_id in request.active_agents},
        }
        return result
