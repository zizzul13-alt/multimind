...
# Stable product vocabulary. Empty route_order means the identity becomes
# available only when runtime discovery contributes a truthful route.
...
# Preserve the closed primary catalogue contract while allowing runtime discovery
# to expose additional identities through a separate vocabulary.
...
def runtime_identity_specs(agents: Mapping[str, object] | None) -> dict[str, AiIdentitySpec]:
    ...
    route_map = {key: list(spec.route_order) for key, spec in AI_IDENTITIES.items()}
    for route_id, provider in configured.items():
        ...
        if identity_id in route_map:
            route_map[identity_id].append(str(route_id))
    return {
        identity_id: AiIdentitySpec(base.identity_id, base.display_name, base.families, tuple(dict.fromkeys(route_map[identity_id])), base.execution_slot)
