...
# Known identities are presentation vocabulary. Empty route_order means the AI is
# selectable vocabulary but currently unavailable until discovery supplies a route.
...
_MODEL_IDENTITY_CHECKS = (
    (("gemini",), "gemini"), (("gpt-oss",), "gpt-oss"),
    (("llama", "meta/llama", "meta-llama"), "llama"), (("deepseek",), "deepseek"),
    (("claude", "anthropic/"), "claude"), (("qwen",), "qwen"),
    (("kimi", "moonshot"), "kimi"), (("grok", "x-ai/", "xai/"), "grok"),
    (("gpt-", "openai/gpt-"), "gpt"),
)
...
def runtime_identity_specs(agents: Mapping[str, object] | None) -> dict[str, AiIdentitySpec]:
    ...
    result={}
    for identity_id,base in AI_IDENTITIES.items():
        result[identity_id]=AiIdentitySpec(base.identity_id,base.display_name,base.families,tuple(dict.fromkeys(route_map[identity_id])),base.execution_slot)
    return result

class IdentityRoutedProvider(BaseProvider):
    def __init__(self,spec:AiIdentitySpec,routes:list[tuple[str,BaseProvider]]):
        super().__init__(spec.display_name); self.spec=spec; self.routes=list(routes); self.model_name=spec.display_name
    def generate(self,prompt:str,system_prompt:str=None,mode:str="coding",max_tokens:int=4096,**kwargs)->dict:
        ...
        for route_index,(route_id,provider) in enumerate(self.routes):
            try: response=provider.generate(prompt=prompt,system_prompt=system_prompt,mode=mode,max_tokens=max_tokens,**kwargs)
                ...
                attempts.append({"route":route_id,"status":"error","reason":type(exc).__name__}); continue
            if not BaseProvider.has_usable_response(response):
                attempts.append({"route":route_id,"status":"error","reason":str(response.get("failure_category") or "provider_error") if isinstance(response,dict) else "provider_error"}); continue
...
def available_identity_options(agents:Mapping[str,object]|None)->list[str]: return list(build_identity_providers(agents))
