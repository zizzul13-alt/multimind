...
def build_database_for_user(user_id,database_factory=DatabaseManager,*,environ=None,turso_factory=TursoDatabaseManager):
    """Construct validated user-scoped persistence."""
    user_id=Config.validate_user_id(user_id)
    if database_factory is not DatabaseManager: return database_factory(Config.get_db_path(user_id))
    environ=os.environ if environ is None else environ
    turso_url=environ.get("TURSO_DATABASE_URL","").strip(); turso_token=environ.get("TURSO_AUTH_TOKEN","").strip()
    if bool(turso_url)!=bool(turso_token): raise RuntimeError("TURSO_DATABASE_URL and TURSO_AUTH_TOKEN must be configured together.")
    if turso_url and turso_token: return turso_factory(turso_url,turso_token,user_id)
    return database_factory(Config.get_db_path(user_id))


def build_application_for_user(user_id,secrets_source=None,*,allow_default_credentials=True,runtime_memories=None,runtime=None,db=None,db_factory=None,agents=None,agents_factory=build_agents,database_factory=DatabaseManager,compressor=PromptCompressor,file_handler=FileHandler,debate_factory=DebateOrchestrator,persist_chat=persist_chat_and_update_memory):
    """Build one user-scoped conversation-first application for any presentation host."""
    user_id=Config.validate_user_id(user_id)
    if agents is None:
        api_keys=Config.get_api_keys(user_id,secrets_source=secrets_source,allow_default=allow_default_credentials); agents=agents_factory(api_keys)
    if db is None and db_factory is None: db=build_database_for_user(user_id,database_factory=database_factory)
    return ConversationFirstApplication(agents=agents,runtime_memories=runtime_memories,runtime=runtime,db=db,db_factory=db_factory,compressor=compressor,file_handler=file_handler,debate_factory=debate_factory,persist_chat=persist_chat)
