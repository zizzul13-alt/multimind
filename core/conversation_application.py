...
from core.operating_model import select_auto_identities,select_work_mode
...
class _RetrievalMemory:
    ...
    def get_context(self):
        current=self.base.get_context() if self.base is not None and hasattr(self.base,"get_context") else ""
        parts=[part for part in (current, f"RETRIEVED RELEVANT HISTORY:\n{self.retrieved}" if self.retrieved else "") if part]
        return "\n\n".join(parts)
...
class ConversationFirstApplication(IdentityFirstApplication):
    ...
    def execute_chat(self,request:ChatRequest):
        mode_policy=str(getattr(request,"work_mode_policy","manual") or "manual")
        mode_selection=select_work_mode(request.original_prompt,request.session_mode,mode_policy)
        ai_policy=str(getattr(request,"ai_selection_policy","manual") or "manual").lower()
        authority=str(getattr(request,"authority","think") or "think")
        explicit=list(dict.fromkeys(request.active_agents or []))
        available=available_identity_options(self.agents)
        if ai_policy=="auto":
            count=max(1,min(6,int(getattr(request,"auto_ai_count",1) or 1)))
            selected=select_auto_identities(mode_selection["selected"],available,count=count)
        else:
            selected=explicit
        ...
        if request.session_id:
            task_state=service.crystallize(request.session_id,request.original_prompt,authority)
            retrieved=service.context(request.session_id,request.original_prompt,limit=6)
