...
class DeepSeekProvider(BaseProvider):
    def __init__(self,api_key:str):
        super().__init__("DeepSeek"); self._catalog=None
        if not api_key:
            self.client=None; self.model_name="DeepSeek (not configured)"; self.set_availability(False,"API key not configured"); return
        try:
            self.client=OpenAI(api_key=api_key,base_url="https://api.deepseek.com",timeout=Config.API_TIMEOUT,max_retries=0); self.model_name="DeepSeek (dynamic model)"; self.set_availability(True)
        except Exception as e:
            self.client=None; self.model_name="DeepSeek (error)"; self.set_availability(False,type(e).__name__)
    def _discover(self):
    ...
    def generate(self,prompt:str,system_prompt:str=None,mode:str="coding",max_tokens:int=2000,**kwargs)->dict:
        ...
        resolution=resolve_model("deepseek",mode,discover=self._discover,allow_paid=True)
        ...
        try:
            ...
            usage=getattr(response,"usage",None); input_tokens=usage.prompt_tokens if usage else 0; output_tokens=usage.completion_tokens if usage else 0
            actual_model=getattr(response,"model",None) or resolution.model_id; self.model_name=f"DeepSeek ({actual_model})"; self.set_availability(True)
