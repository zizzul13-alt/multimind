...
class GroqProvider(BaseProvider):
    """Groq protocol adapter; model choice is resolved at call time."""
    ...
    def __init__(self, api_key: str):
        ...
        if not api_key:
            ...
            return
        try:
            self.client = OpenAI(api_key=api_key, base_url="https://api.groq.com/openai/v1", timeout=Config.API_TIMEOUT, max_retries=0)
            self.model_name = "Groq (dynamic model)"
    ...
    def generate(self, prompt: str, system_prompt: str = None, mode: str = "coding", max_tokens: int = 4096, **kwargs) -> dict:
        ...
        resolution = resolve_model("groq", mode, discover=lambda: openai_catalog(self.client))
        ...
        try:
            ...
            self.model_name = f"Groq ({actual_model})"
            self.set_availability(True)
