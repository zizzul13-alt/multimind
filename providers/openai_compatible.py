...
def normalize_resource_spec(spec: Mapping[str, object] | OpenAICompatibleResourceSpec) -> OpenAICompatibleResourceSpec | None:
    ...
    if not candidate.resource_id or not candidate.api_key or parsed.scheme not in {"http", "https"} or not parsed.netloc:
        return None
    return candidate
...
class OpenAICompatibleProvider(BaseProvider):
    ...
    def __init__(self, spec: OpenAICompatibleResourceSpec, model_id: str, *, client=None):
        ...
        self.client = client or OpenAI(api_key=spec.api_key, base_url=spec.base_url, timeout=Config.API_TIMEOUT, max_retries=0)
    ...
    def generate(self, prompt: str, system_prompt: str = None, mode: str = "coding", max_tokens: int = 4096, **kwargs) -> dict:
        ...
        try:
            response = self.client.chat.completions.create(model=self.discovered_model_id, messages=messages, max_tokens=max_tokens)
            ...
            return {"status": "success", "text": text, "agent": self.resource_label, "tokens": tokens, "cost": 0.0, "resolved_model": self.discovered_model_id, "actual_model": actual_model, "model_resolution_source": "provider_catalog_discovery", "identity_provenance": self.identity_provenance, "resource_id": self.resource_id}
        except Exception as exc:
            self.set_availability(False, type(exc).__name__)
            error_logger.log("PROVIDER_FAILURE", f"provider=OpenAICompatible resource={self.resource_id} category=network_or_sdk_exception exception_type={type(exc).__name__}")
            return self.failure_response("network_or_sdk_exception", exception_type=type(exc).__name__)


def discover_resource_providers(spec: Mapping[str, object] | OpenAICompatibleResourceSpec, *, client_factory=OpenAI, max_models: int = 128) -> dict[str, OpenAICompatibleProvider]:
    """Discover endpoint-declared models; failure contributes zero routes."""
    normalized = normalize_resource_spec(spec)
    if normalized is None:
        return {}
    try:
        client = client_factory(api_key=normalized.api_key, base_url=normalized.base_url, timeout=Config.API_TIMEOUT, max_retries=0)
        model_ids = openai_catalog(client)
    except Exception as exc:
        error_logger.log("PROVIDER_DISCOVERY_FAILURE", f"resource={normalized.resource_id} exception_type={type(exc).__name__}")
        return {}
    routes = {}
