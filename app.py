...
def show_sidebar():
    with st.sidebar:
        ...
        with st.expander("⚙️ Settings"):
            ...
            st.session_state.active_agents = st.multiselect("Agents", ["gemini", "deepseek", "groq"], default=st.session_state.active_agents, key="settings_agents")
        st.divider()
...
def process_chat(prompt, uploaded_files, context_mode):
    ...
    if not gemini and not deepseek and not groq:
...
def main():
    ...
    if st.session_state.user is None:
        ...
        if st.session_state.current_session:
            ...
            ### Features:
            - 🤖 Multi-agent debate (Groq + DeepSeek + Gemini)
