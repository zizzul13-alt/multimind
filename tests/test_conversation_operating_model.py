...
def test_retrieval_prefers_relevant_history():
    rows = [
        {"prompt": "music discovery", "final_answer": "track list"},
        {"prompt": "MultiMind memory architecture", "final_answer": "SQLite retrieval"},
    ]
...
class FakeDb:
    def __init__(self):
        ...
        self.rows = {
            ...
            "s2": [
                {
                    "session_id": "s2",
                    "prompt": "MultiMind provider routing",
                    "final_answer": "identity truth",
                    "debate_data": "{}",
                }
            ],
        }
    ...
    def get_session_chats(self, session_id, limit=50):
        return list(self.rows[session_id])[:limit]
...
def test_task_checkpoint_reads_last_persisted_product_semantics():
    db = FakeDb()
    task = {"objective": "ship it", "authority": "think", "status": "active"}
    db.rows["s1"].append(
        {
            "session_id": "s1",
            "prompt": "gas",
            "final_answer": "done",
            "debate_data": json.dumps({"product_semantics": {"task_state": task}}),
        }
    )
    assert ConversationStateService(db).task_state("s1") == task


def test_checkpoint_is_verified_and_survives_sqlite_reopen(tmp_path):
    db_path = tmp_path / "checkpoint.db"
    db = DatabaseManager(str(db_path))
    db.create_session("s1", "checkpoint proof", "coding")
    db.save_chat(
        "s1",
        {
            "id": "c1",
            "prompt": "gas implement bounded fix",
            "final_answer": "done",
            "debate_data": "{}",
        },
    )
    ...
    debate_data = {
        "product_semantics": {
            "work_mode": {"selected": "coding", "reason": "auto_inference"},
            "ai_selection": {"policy": "auto", "selected": ["gemini"]},
            "authority": "think",
            "task_state": task,
        }
    }

    assert ConversationStateService(db).persist_checkpoint("s1", "c1", debate_data)

    reopened = DatabaseManager(str(db_path))
    persisted = json.loads(reopened.get_chat("s1", "c1")["debate_data"])
    assert persisted["product_semantics"] == debate_data["product_semantics"]
    assert ConversationStateService(reopened).task_state("s1") == task


def test_checkpoint_fails_closed_when_update_does_not_land():
    class BrokenDb:
        def update_chat_debate_data(self, session_id, chat_id, debate_data):
            return False

        def get_chat(self, session_id, chat_id):
            raise AssertionError("readback must not happen after a failed write")

    assert not ConversationStateService(BrokenDb()).persist_checkpoint(
        "s1", "c1", {"product_semantics": {"authority": "think"}}
    )
