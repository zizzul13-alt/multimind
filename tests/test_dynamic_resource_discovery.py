...
from core.ai_identity import AI_IDENTITY_OPTIONS, available_identity_options, build_identity_providers, infer_ai_identity, runtime_identity_specs
...
def _spec():
    return {"id": "kios-a", "label": "Kios A", "base_url": "https://kios.example/v1", "api_key": "secret"}
...
def test_discovery_classifies_known_families_but_leaves_unknown_inert():
    ...
    assert "mystery-premium" not in identities
    assert set(available_identity_options(routes)) == {"claude", "qwen"}


def test_primary_identity_catalogue_remains_compatible():
    assert AI_IDENTITY_OPTIONS == ("gemini", "gpt-oss", "llama", "deepseek")
