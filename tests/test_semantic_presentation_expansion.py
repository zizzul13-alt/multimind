...
def test_t14_every_s84_css_variable_has_active_selector_consumer():
    """T14: Verifies that every S8.4 CSS custom property generated by generate_theme_css is actually consumed by active selectors in ui/style.css."""
    with open("ui/style.css", "r", encoding="utf-8") as f:
        style_css = f.read()

    s84_css_variables = [
        "--mm-heading-font-weight",
        "--mm-heading-letter-spacing",
        "--mm-shape-border-style",
        "--mm-shape-border-width",
        "--mm-shape-border-color",
        "--mm-energy-hover-lift",
        "--mm-energy-hover-shadow",
        "--mm-surface-elevation-shadow",
        "--mm-transition-spec",
    ]

    for var in s84_css_variables:
        assert var in style_css, f"CSS custom property {var} generated by Theme Engine is missing from active selectors in ui/style.css"
