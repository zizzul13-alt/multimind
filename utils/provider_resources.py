...
_PROVIDER_TO_KEY = {
    "gemini": "gemini_key", "deepseek": "deepseek_key", "groq": "groq_key",
    "openrouter": "openrouter_key", "huggingface": "huggingface_key", "remote": "remote_url",
}
...
def _selected_resource(spec):
    ...
    if not isinstance(default_name, str) or not default_name or not isinstance(resources, Mapping):
        return None
...
def _compatible_resource(provider_name: str, resource) -> dict[str, str] | None:
    ...
    return {
        "id": resource_id,
        "label": str(resource.get("label") or provider_name).strip(),
...
def parse_user_provider_pools(raw: str | None) -> dict[str, dict[str, object]]:
    """Parse per-user resource pools into adapter settings, failing closed."""
    ...
    for supplied_user, providers in document.items():
        ...
        for provider_name, spec in providers.items():
            ...
            if provider == "cloudflare":
                if not isinstance(resource, Mapping):
                    continue
                key, account_id = resource.get("key"), resource.get("account_id")
